-- Complete the default WEB layout navigation hierarchy. V6 created the
-- group/module/categories, but no feature groups or features; the navigation
-- service prunes every empty branch and consequently returned navTree: [].
...
-- Link features to privileges when the privilege catalog already exists.
-- On a new installation the application catalog is populated after Flyway;
-- unlinked features remain visible to authenticated users rather than being
-- pruned, while API authorization remains enforced independently.
