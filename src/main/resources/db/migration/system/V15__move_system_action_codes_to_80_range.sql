-- Move system-administration actions from 90-97 to the reserved 80-87 range.
-- V14 has already been applied in existing environments and must remain immutable.

CREATE TEMP TABLE system_action_code_migration (
    old_code varchar(2) PRIMARY KEY,
    new_code varchar(2) NOT NULL UNIQUE,
    action_name varchar(255) NOT NULL
) ON COMMIT DROP;

INSERT INTO system_action_code_migration (old_code, new_code, action_name)
VALUES
    ('90', '80', 'Synchronize'),
    ('91', '81', 'Assign'),
    ('92', '82', 'Generate'),
...
CREATE TEMP TABLE duplicate_system_privilege_migration AS
SELECT legacy.id AS legacy_id, canonical.id AS canonical_id
FROM sys_priv_privileges legacy
JOIN system_action_code_migration mapping
...
JOIN duplicate_system_privilege_migration duplicate ON duplicate.legacy_id = assignment.privilege_id
ON CONFLICT (role_id, privilege_id) DO NOTHING;

DELETE FROM sys_priv_role_privileges assignment
USING duplicate_system_privilege_migration duplicate
...
JOIN duplicate_system_privilege_migration duplicate ON duplicate.legacy_id = permission.privilege_id
...
SET privilege_id = duplicate.canonical_id,
    updated_by = 0,
    updated_at = CURRENT_TIMESTAMP
FROM duplicate_system_privilege_migration duplicate
...
SET required_privilege_code = left(registry.required_privilege_code, 9) || mapping.new_code,
    updated_by = 0,
    updated_at = CURRENT_TIMESTAMP
FROM system_action_code_migration mapping
