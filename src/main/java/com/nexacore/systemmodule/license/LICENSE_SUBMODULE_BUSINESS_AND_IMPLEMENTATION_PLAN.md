...
When the entitlement maps to an API, use `sys_api_registry.id`. When it maps to module, submodule, feature, or action access, use the normalized privilege catalog IDs instead of copying code/name values into license tables.

Use the normalized privilege catalog instead of duplicating module, submodule, and feature names:

| Entitlement type | Reference |
| --- | --- |
| `MODULE` | `sys_modules.id` |
| `SUBMODULE` | `sys_submodules.id` |
| `FEATURE` | `sys_features.id` |
| `ACTION` | `sys_privileges.id` |
| `API` | `sys_api_registry.id` |
...
- `client_application_id` should reference `sys_client_applications.id` when licensing a specific frontend or integration client.
...
- Add foreign keys to `sys_client_applications`, `sys_api_registry`, and `sys_privileges` where applicable.
